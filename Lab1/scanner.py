...
class Scanner:
    ...
    def __init__(self, fileName):
        ...
        self.outputConstantsTable = path.splitext(path.basename(fileName))[
            0] + "_const_table.txt"
        ...
        for key in tableToSort:
            for i in range(0, len(dictValuesSorted) - 1):
                if dictValuesSorted[i] == key:
                    newDict[key] = [tableToSort[key],
                                    tableToSort[dictValuesSorted[i + 1]]]
            ...
            while True:
                ...
                if ch.isalpha():
                    ...
                    if not self.identifierPattern.match(_id):
                        print(
                            "ERROR: Identifier has too many characters. (line, col) = (%d, %d)" % (i, j))
                        sys.exit()
